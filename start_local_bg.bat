...
if !errorlevel! neq 0 (
    if exist "%~dp0.local\redis\redis-server.exe" (
        echo   Starting Redis...
        start "IDEP-Redis" start /b cmd /c """%~dp0.local\redis\redis-server.exe"" --port 6379"
...
if !errorlevel! neq 0 (
    if exist "%~dp0.local\bin\temporal.exe" (
        ...
        start "IDEP-Temporal" start /b cmd /c """%~dp0.local\bin\temporal.exe"" server start-dev --ip 127.0.0.1 --port 7233 --ui-port 8233"
...
if !errorlevel! neq 0 (
    ...
    start "IDEP-Preprocessing" start /b cmd /c call "%~dp0run_with_env.bat" go run main.go
    popd
...
if !errorlevel! neq 0 (
    ...
    start "IDEP-PostProcessing" start /b cmd /c call "%~dp0run_with_env.bat" python main.py
    popd
    timeout /t 2 /nobreak >nul
...
if !errorlevel! neq 0 (
    ...
    start "IDEP-GLM-OCR" start /b cmd /c call "%~dp0run_with_env.bat" python -m uvicorn app.main:app --host 0.0.0.0 --port 8002
    popd
    timeout /t 3 /nobreak >nul
...
start "IDEP-Worker" start /b cmd /c call "%~dp0run_with_env.bat" go run worker/main.go
popd
timeout /t 3 /nobreak >nul
...
if !errorlevel! neq 0 (
    ...
    start "IDEP-API-Gateway" start /b cmd /c call "%~dp0run_with_env.bat" go run main.go
    popd
    timeout /t 5 /nobreak >nul
